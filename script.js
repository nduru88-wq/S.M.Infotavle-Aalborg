...
var PERSONALE_API =
  "https://script.google.com/macros/s/AKfycby5jnw7b_PJHvYedTjOvmalA-0ldvnOsWwwIhSRlCEO3emxZochj9uhZvxV7LPK_u4mdA/exec?action=personale";

function hentPersonaleBillederFraDrive() {
  var callbackName =
    "smPersonaleCallback_" +
    Date.now() +
    "_" +
  ...
  window[callbackName] = function(data) {
    try {
      if (
        ...
        throw new Error("Ugyldigt svar fra personale-billedserveren");
      }

      Object.keys(data.billeder).forEach(function(navn) {
        var person = navn.charAt(0).toUpperCase() + navn.slice(1);
        var info = data.billeder[navn];

        if (info && info.url) {
          PERSON_BILLEDER[person] = info.url;
        }
      });
      ...
      console.log(
        "Kunne ikke bruge personale-billeder fra Drive:",
        err
      );
    } finally {
  ...
  script.onerror = function() {
    console.log(
      "Kunne ikke hente personale-billeder fra Drive"
    );
